...
to create-and-remove-cars
  ...
  if count cars-heading-east < cars-going-east [ ; create one car heading east, if needed
    create-cars 1 [
      ...
      setxy min-pxcor one-of [-1 -3]
      let this-car self
      set color car-color
      set heading 90
      set top-speed 0.005 + random-float 0.05
      set speed 0.05 + random-float 0.01
...
to slow-down
  ...
  ; brake / reduce speed based on deceleration variable
  ; reduce patience by 1 everytime hit brake
end
